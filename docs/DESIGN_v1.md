...
The cosmic True Random Number Generation (cTRNG) is an orbital service that provides true random numbers harvested from hardware on satellites. Orbitport's current upstream integration targets the `Crypto2` API; `cEdge` will be integrated separately.
...
1. **crypto2:** Cosmic True Random Number Generation source provided by the Crypto2 satellite API.
