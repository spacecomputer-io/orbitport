# Orbitport | Dev Docs

This folder contains internal/dev documentation for Orbitport.


## Overview

Orbitport is a gateway to orbital services such as `cTRNG` (cosmic True Random Number Generator) or `spaceTEE`, served by multiple providers & satellites to ensure high availability and reliability.

Orbitport is designed to be easy to use and integrate with multiple types of web3 and web2 systems, making it an ideal choice for developers and organizations looking to leverage the power of space-based services.

### Services

Orbitport currently supports the following services:

#### 1. cTRNG

The cosmic True Random Number Generation (cTRNG) is an orbital service that provides true random numbers harvested from hardware on satellites. Orbitport's current upstream integration is the `Crypto2` API, and `cEdge` will be integrated separately.
The satellite also signs the generated data to ensure it was generated by the satellite and was not tampered with.

#### 2. spaceTEE

The space Trusted Execution Environment (TEE) is an orbital service that provides a secure enclave environment for running code in space, ensuring the integrity and confidentiality of the code and data.

## Links

- [Roadmap](ROADMAP.md)
- [Design v1](DESIGN_v1.md)
- [Dev Guide](DEV_GUIDE.md)

## External Links

- [Orbitport user guide](https://docs.spacecomputer.io/using-orbitport/user-guide)
- [SpaceComputer docs](https://docs.spacecomputer.io)
