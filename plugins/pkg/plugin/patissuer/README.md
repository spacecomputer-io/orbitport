...
| RPC | Purpose |
| --- | --- |
| `IssueToken(jti, subject, kms_tenant, expires_at)` | Returns the compact JWS. Fail-closed validation: jti/subject required, expiry must be future and under `ORBITPORT_PATISSUER_MAX_TTL_DAYS` |
| `GetJwks()` | RFC 7517 JWK Set (public keys only), kid-selected by verifiers |

## Key custody (`ORBITPORT_PATISSUER_SIGNER`)

- `local` (default): EC P-256 key in process memory, from
  `ORBITPORT_PATISSUER_LOCAL_KEY_PEM` (SEC1 or PKCS#8 PEM) or generated
...
- `transit`: OpenBao Transit via the OpenBao proxy — hash-then-sign
  ...
  compose stack (`ORBITPORT_PATISSUER_LIVE_TEST=1 go test ./pkg/plugin/patissuer/
