...
// This is the primitive used with a per-seed cursor: plugin decides the offset, from which bytes are produced.
func (m MasterSeed) DeriveBulkAtOffset(n int, offsetBytes uint64, outLen int) ([]string, error) {
	if n <= 0 {
		return []string{}, nil
	}
...
func (m MasterSeed) DeriveBulk(n int, outLen int) ([]string, error) {
	return m.DeriveBulkAtOffset(n, 0, outLen)
}
