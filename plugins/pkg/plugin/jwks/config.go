...
func readFromEnv() *jwksConfig {
	...
	return &jwksConfig{
		PatIssuerPlugin: strings.TrimSpace(viper.GetString("JWKS_PATISSUER_PLUGIN")),
...
func setDefaults() {
	viper.SetDefault("JWKS_PATISSUER_PLUGIN", "")
...
func (c *jwksConfig) validate() error {
	if c.PatIssuerPlugin == "" {
		return fmt.Errorf("FATAL: ORBITPORT_JWKS_PATISSUER_PLUGIN is required")
	}
	if c.HTTPPort == 0 {
