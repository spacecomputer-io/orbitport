...
pub async fn start(
    ...
    let auth_client = match plugin_catalog.get_auth_client().await {
        Ok(client) => client,
        Err(e) => {
            tracing::error!("Failed to get auth client: {}", e);
            return;
        }
    };
    let auth_client_cloned = auth_client.clone();
    ...
    let rpc_route = warp::post()
        ...
        .and(with_account_hold(
            with_rate_limiter(with_auth(auth_client_cloned), rate_limiter.clone()),
    ...
    if let Some(client) = patissuer_client {
        let internal = internal_routes(client, auth_client.clone());
