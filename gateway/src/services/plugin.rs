...
//!   "params": {
//!     "plugin": "astros",
//!     "method": "astros.AstrosPlugin.GetPeopleInSpace",
//!     "request": {}
//!   }
//! }
//! ```
...
/// Emitted by `build.rs`; see `file_descriptor_set_path` there.
...
pub struct PluginCallRequest {
    /// Catalog key of the target plugin (e.g. `"astros"`) — must match an
    /// `ORBITPORT_PLUGIN_<NAME>` environment variable.
    pub plugin: String,
...
pub async fn dispatch(
    ...
    let (service_name, method_name) = call.method.rsplit_once('.').ok_or_else(|| {
        Status::invalid_argument(
            "method must be fully qualified: package.Service.Method",
        )
    })?;
    ...
    // JSON → DynamicMessage using the method's input descriptor.
    let input_desc = method.input();
    let input_msg = {
        let json = serde_json::to_string(&call.request).map_err(|e| {
            Status::internal(format!("failed to stringify request params: {e}"))
        })?;
    ...
    // Dial the plugin by catalog key.
    let channel = catalog
